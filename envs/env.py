...
class Env(object):
    ...
    def run(self, is_training=False):
        ...
        while not self.is_over():
            # Agent plays
            if not is_training:
                eval_out = self.agents[player_id].eval_step(state)
                action = eval_out[0] if isinstance(eval_out, tuple) else eval_out
            else:
                action = self.agents[player_id].step(state)

            # Environment steps
            use_raw = getattr(self.agents[player_id], 'use_raw', False)
            next_state, next_player_id = self.step(action, use_raw)
            # Save action
    ...
    def seed(self, seed=None):
        # Gymnasium-style semantics:
        # - If seed is provided, it must be a non-negative int.
        # - If seed is None, derive entropy from system randomness.
        if seed is not None:
            if not isinstance(seed, int) or seed < 0:
                raise ValueError(f'Seed must be a non-negative int or None, got {seed!r}')
            final_seed = seed
        else:
            final_seed = random.SystemRandom().randrange(2**32)

        # Keep RandomState for backward compatibility with engine code that uses
        # legacy APIs such as randint.
        self.np_random = np.random.RandomState(final_seed)
        self.py_random = random.Random(final_seed)
